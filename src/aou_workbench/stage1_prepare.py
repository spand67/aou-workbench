...
from .io_utils import ensure_parent_dir, query_bigquery_dataframe, write_dataframe, write_json
...
def _wgs_sample_manifest_sql(config: ProjectConfig) -> str:
...
def prepare_wgs_sample_manifest(config: ProjectConfig) -> pd.DataFrame:
    """Write the CDR WGS sample manifest used by downstream WGS restrictions."""
    config = apply_runtime_defaults(config)
    stage = config.analysis.stage1
    if stage is None:
    ...
    sql = _wgs_sample_manifest_sql(config)
    print("Reading WGS availability from cb_search_person.has_whole_genome_variant", flush=True)
    frame = query_bigquery_dataframe(sql)
    manifest = _sample_manifest_frame(frame["person_id"].astype(str).tolist() if "person_id" in frame.columns else [])
    ...
    write_json(
        {
            ...
            "source_table": f"{config.workbench.workspace_cdr}.cb_search_person",
            "source_field": "has_whole_genome_variant",
        },
        metadata_path,
    )
    print(f"Wrote {len(manifest)} WGS sample IDs to {sample_manifest_path}", flush=True)
    return manifest
