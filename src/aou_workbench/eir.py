...
def _cofactor_condition_predicate(config: ProjectConfig, names: tuple[str, ...]) -> str:
    predicates: list[str] = []
    for rule in config.phenotype.clinical_cofactors:
...
def _lab_measurement_predicate(config: ProjectConfig, lab: str) -> str:
    terms = config.phenotype.definite.measurement_terms if lab == "ck" else LAB_TERMS[lab]
    ids = config.phenotype.definite.measurement_concept_ids if lab == "ck" else ()
    return _match_predicate(
        concept_id_column="measurement_concept_id",
        concept_name_column="measurement_concept_name",
        concept_ids=ids,
        concept_terms=terms,
    )
...
def _build_eir_cohort_bigquery_aggregated(
    ...
    *,
    ...
    feature_predicates = {
        "prior_kidney_disease_or_aki": _cofactor_condition_predicate(config, ("kidney_disease", "renal_injury")),
        "remote_sepsis_history": sepsis_predicate,
        "prior_heat_illness_or_dehydration": _cofactor_condition_predicate(config, ("heat_illness", "dehydration")),
        "prior_exertion_or_exercise_code": _cofactor_condition_predicate(config, ("exertion_exercise",)),
        "prior_myopathy_muscle_disease": _cofactor_condition_predicate(config, ("myopathy_muscle_disease",)),
        "prior_sickle_trait_or_disease": _cofactor_condition_predicate(config, ("sickle_cell_trait_or_disease",)),
        "prior_diabetes": _cofactor_condition_predicate(config, ("diabetes",)),
        "prior_thyroid_disease": _cofactor_condition_predicate(config, ("thyroid_disease",)),
        "prior_liver_disease": _cofactor_condition_predicate(config, ("liver_disease",)),
        "prior_alcohol_substance_condition": _cofactor_condition_predicate(config, ("alcohol_substance",)),
        "prior_injury_trauma_history": _cofactor_condition_predicate(config, ("prior_injury_trauma", "major_trauma", "crush_injury")),
    }
    feature_selects = [
        f"IF(COUNTIF(condition_date < pt.baseline_date AND ({predicate})) > 0, 1, 0) AS {column}"
        for column, predicate in feature_predicates.items()
    ]

    lab_predicates = {lab: _lab_measurement_predicate(config, lab) for lab in LAB_TERMS}
    lab_case = "\n    ".join(
        f"WHEN ({predicate}) THEN '{_escape_term(lab)}'"
        for lab, predicate in lab_predicates.items()
        if predicate != "FALSE"
    )
    lab_filter = " OR ".join(f"({predicate})" for predicate in lab_predicates.values() if predicate != "FALSE") or "FALSE"
    lab_selects = []
    for lab in LAB_TERMS:
        lab_selects.extend(
            [
                (
                    "ARRAY_AGG("
                    f"IF(lab = '{_escape_term(lab)}', STRUCT(measurement_date AS date, measurement_value AS value), NULL) "
                    "IGNORE NULLS ORDER BY measurement_date DESC LIMIT 1"
                    f")[SAFE_OFFSET(0)].value AS prebaseline_{lab}_last"
                ),
                f"MAX(IF(lab = '{_escape_term(lab)}', measurement_value, NULL)) AS prebaseline_{lab}_max",
                f"COUNTIF(lab = '{_escape_term(lab)}') AS prebaseline_{lab}_n",
            ]
        )
...
person_index AS (
  SELECT pt.*, rhabdo_rollup.prior_rhabdo_date, rhabdo_rollup.first_rhabdo_date, COALESCE(rhabdo_rollup.rhabdo_during_horizon, 0) AS rhabdo_during_horizon
  FROM person_time pt
...
ck_near_index AS (
  SELECT
    pt.person_id,
    ARRAY_AGG(
      STRUCT(measurement_date AS date, measurement_value AS value)
      ORDER BY ABS(DATE_DIFF(measurement_date, pt.first_rhabdo_date, DAY)), measurement_date
      LIMIT 1
    )[SAFE_OFFSET(0)] AS ck_confirming
  FROM person_index pt
  JOIN measurement_records
    ON pt.person_id = measurement_records.person_id
  WHERE pt.first_rhabdo_date IS NOT NULL
    AND ({ck_predicate})
    AND measurement_value >= {ck_threshold}
    AND DATE_DIFF(measurement_date, pt.first_rhabdo_date, DAY) BETWEEN {config.phenotype.definite.measurement_window_start_days} AND {config.phenotype.definite.measurement_window_end_days}
  GROUP BY pt.person_id
),
cofactor_rollup AS (
  SELECT
    ...
    IF(COUNTIF(({sepsis_predicate}) AND condition_date BETWEEN DATE_ADD(pt.first_rhabdo_date, INTERVAL {SEPSIS_EXCLUSION_START_DAYS} DAY) AND DATE_ADD(pt.first_rhabdo_date, INTERVAL {SEPSIS_EXCLUSION_END_DAYS} DAY)) > 0, 1, 0) AS excluded_pre_or_same_day_sepsis,
...
prebaseline_labs AS (
  ...
  JOIN measurement_records
    ON pt.person_id = measurement_records.person_id
  WHERE measurement_date < pt.baseline_date
...
assembled AS (
  SELECT
    ...
    ck_near_index.ck_confirming.date AS ck_confirming_date,
    ck_near_index.ck_confirming.value AS ck_confirming_value,
    IF(ck_near_index.ck_confirming.value >= {ck_threshold}, 1, 0) AS ck_confirmed_index,
    COALESCE(cofactor_rollup.excluded_periindex_trauma, 0) AS excluded_periindex_trauma,
    COALESCE(cofactor_rollup.excluded_pre_or_same_day_sepsis, 0) AS excluded_pre_or_same_day_sepsis,
    COALESCE(cofactor_rollup.supportive_exertion_heat_dehydration_code, 0) AS supportive_exertion_heat_dehydration_code,
    {", ".join(f"COALESCE(cofactor_rollup.{column}, 0) AS {column}" for column in feature_predicates)},
    lab_rollup.* EXCEPT(person_id)
  FROM person_index pt
  LEFT JOIN ck_rollup USING (person_id)
  LEFT JOIN ck_near_index USING (person_id)
  LEFT JOIN cofactor_rollup USING (person_id)
  LEFT JOIN lab_rollup USING (person_id)
),
finalized AS (
  SELECT
    assembled.*,
...
SELECT
  finalized.*,
  SAFE_CAST(age_raw AS FLOAT64) AS age_at_baseline,
  IF(incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0, 1, 0) AS eir_diagnosis_only_case,
  IF(incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND ck_confirmed_index = 1 AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0, 1, 0) AS eir_primary_case,
  IF(incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND ck_confirmed_index = 1 AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0 AND supportive_exertion_heat_dehydration_code = 1, 1, 0) AS eir_exertion_heat_dehydration_coded_case,
  IF(incident_denominator = 1 AND rhabdo_during_horizon = 0 AND high_ck_during_horizon = 1, 1, 0) AS ck_only_no_rhabdo,
  IF(obs_end_date >= horizon_end_date AND (last_clinical_activity_date IS NULL OR last_clinical_activity_date >= baseline_date), 1, 0) AS observed_followup_through_horizon,
  IF(
    incident_denominator = 1
    AND obs_end_date >= horizon_end_date
    AND (last_clinical_activity_date IS NULL OR last_clinical_activity_date >= baseline_date)
  ...
  IF(
    ...
    1,
    0
  ) AS eir_ck_tested_control,
  CASE
    WHEN incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND ck_confirmed_index = 1 AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0 THEN 1
    ELSE 0
  END AS analysis_case,
  CASE
    WHEN incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND ck_confirmed_index = 1 AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0 THEN 'eir_primary'
    WHEN incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0 THEN 'diagnosis_only'
    WHEN incident_denominator = 1 AND rhabdo_during_horizon = 0 AND high_ck_during_horizon = 1 THEN 'ck_only_no_rhabdo'
    WHEN incident_denominator = 1 AND obs_end_date >= horizon_end_date AND rhabdo_during_horizon = 0 AND high_ck_during_horizon = 0 THEN 'control'
    ELSE 'excluded_or_unclassified'
  END AS case_tier,
  IF(
    (
      incident_denominator = 1 AND first_rhabdo_date IS NOT NULL AND ck_confirmed_index = 1 AND excluded_periindex_trauma = 0 AND excluded_pre_or_same_day_sepsis = 0
    )
    OR (
      incident_denominator = 1
      AND obs_end_date >= horizon_end_date
...
""".strip()
    ...
    integer_columns = [
        ...
        "excluded_pre_or_same_day_sepsis",
        "supportive_exertion_heat_dehydration_code",
        *feature_predicates.keys(),
        "incident_denominator",
        "eir_diagnosis_only_case",
        "eir_primary_case",
        "eir_exertion_heat_dehydration_coded_case",
        "ck_only_no_rhabdo",
        "observed_followup_through_horizon",
        "eligible_control",
        "eir_ck_tested_control",
        "analysis_case",
        "case_control_eligible",
    ]
